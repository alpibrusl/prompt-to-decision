...
checks a recommendation against this book's closing checklist -- whether the objective, the alternatives, the threshold and the owner were actually stated anywhere, rather than assumed.
...
"am I ready to decide this?" and having something ask it for you every time.
...
The skill lives in this book's repository, at `skills/verify-decision/`. Download
...
```
~/.claude/skills/verify-decision/
```

That makes it available in every project. To scope it to one project
instead, copy it to `.claude/skills/verify-decision/` inside that project.
...
```
~/.config/opencode/skills/verify-decision/
```
