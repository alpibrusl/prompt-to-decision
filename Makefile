...
# The ledger is source; GLOSSARY.md is derived. Both commands live upstream in
...
# House style, as far as a machine can check it. Warnings only -- prose is not
# a build failure -- so this is not wired into `all`. `make prose-fix` applies
# the mechanical corrections.
prose:
	@bookkit check prose -b .
